...
import { extractIngredients, getProductRecipes, getIngredientSpec, } from './tableReader';
import { guaranteedProductRecipeTable } from './tables';
import { mix } from './mix';
import { create } from './create';
function getMaxScore() {
    return Math.max.apply(null, extractIngredients().map(function (x) { return x.score; }));
}
var UuRobo = /** @class */ (function () {
    ...
    UuRobo.mix = function (firstIngredient, secondIngredient, thirdIngredient, fourthIngredient) {
        return mix(firstIngredient, secondIngredient, thirdIngredient, fourthIngredient);
    };
    /**
        ...
        if (recipes.length > 0) {
            var listReplacedToItemName = recipes.map(function (recipe) {
                ...
                var lists = recipe.lists
                    ...
                    return extractIngredients({ specifiedScore: list[0], type: type }).length > 0;
                })
                    .map(function (list) {
                    return list.map(function (score, i) {
                        return i === 0
                            ? extractIngredients({ specifiedScore: score, type: type })
    ...
    UuRobo.prototype.mix = function () {
        if (this.ingredients.length === 4 &&
            ...
            return mix(this.ingredients[0], this.ingredients[1], this.ingredients[2], this.ingredients[3]);
        }
        else {
            return [];
        }
    ...
    UuRobo.prototype.getRequiredIngredients = function (nth) {
        ...
        var productRecipes = getProductRecipes(this.targetProduct, {
            onlyMinCost: false,
        });
        if (productRecipes.standard) {
            productRecipes.standard.forEach(function (recipe) {
                ...
                if (id > 0) {
                    ...
                    if (firstIngredient) {
                        if (((_a = getIngredientSpec(firstIngredient)) === null || _a === void 0 ? void 0 : _a.type) !== type)
                            return;
                    }
                }
                recipe.scores.forEach(function (score) {
                    ...
                    for (var i = 1; i <= id; i++) {
                        ...
                        if (ingredient) {
                            var spec = getIngredientSpec(ingredient);
                            if (spec) {
                                s += spec.score;
                            }
                        }
                    ...
                    requiredIngredients = __spreadArrays(requiredIngredients, extractIngredients(filterOptions).filter(function (x) {
        ...
        if (productRecipes.ball) {
            productRecipes.ball.forEach(function (recipe) {
                requiredIngredients = __spreadArrays(requiredIngredients, extractIngredients({ specifiedItem: recipe.apricorn }));
            });
        }
